<template>
  <div class="profile-page">
    <div class="content">
      ...
      <el-button @click="navigate('interest')" type="primary">设置喜好地</el-button>
      <h2>出行方式</h2>
      <ul>
        <li v-for="interest in interests2" :key="interest">{{ interest }}</li>
      </ul>
      <el-button @click="navigate('interest2')" type="primary">设置出行</el-button>
      <h2>旅行伙伴</h2>
      <ul>
        <li v-for="interest in interests3" :key="interest">{{ interest }}</li>
      </ul>
      <el-button @click="navigate('interest3')" type="primary">设置旅伴</el-button>
    </div>
  </div>
</template>
...
const avatarUrl = ref(props.data.imgUrl || '');
const gender = ref('');
const age = ref('');
const hometown = ref('');
